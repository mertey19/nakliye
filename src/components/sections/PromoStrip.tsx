 ...
 */
export function PromoStrip({ srcs }: { srcs?: string[] } = {}) {
  const items = srcs
    ? availablePromoPhotos.filter((photo) => srcs.includes(photo.src))
  ...
  return (
    <section aria-labelledby="tanitim-baslik">
      ...
      <p className="mt-5 max-w-2xl text-[16px] leading-relaxed text-ink-700">
        ...
        <span className="font-semibold text-white">İşlerimizden</span>{" "}
      ...
      <ul className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((p) => (
          <li key={p.src}>
            <figure className="overflow-hidden rounded-card border border-line-soft bg-card">
              <Image
                src={p.src}
                alt={p.alt}
                width={p.width}
